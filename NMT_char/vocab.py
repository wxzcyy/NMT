...
#        print(sents)
        chars = [[[self.start_of_word] + [self.char2id[char] for char in word] + [self.end_of_word] for word in sent] for sent in sents]
        return chars
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sent_indices = self.words2charindices(sents)
        sent_padded = torch.tensor(pad_sents_char(sent_indices, self.char2id['<pad>']), device=device) # (b, max_sent_length, max_words_length)
        sent_padded = sent_padded.permute(1,0,2) # (max_sent_length, b, max_words_length)
        return sent_padded
    ...
    def to_input_tensor(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_t = pad_sents(word_ids, self.char2id['<pad>'])
