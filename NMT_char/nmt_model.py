...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device) # Tensor: (src_len, b, max_words_length)
...
#        print(target_padded_chars[1:].view(-1, 21).shape)
        enc_hiddens, dec_init_state = self.encode(source_padded_chars, source_lengths)
        enc_mask = self.generate_sent_masks(enc_hiddens, source_lengths)
        
        # 预测输出 (tgt_len, batch, word_embeddings)
        combined_outputs = self.decode(enc_hiddens, enc_mask, dec_init_state, target_padded_chars)
...
#        print(scores)
        ...
        if self.charDecoder is not None:
            ...
            target_words = target_padded[1:].contiguous().view(-1) # 目标句子里面的单词列表
#            print(target_words)
#            print(f'target_padded_chars.shape:{target_padded_chars.shape}')
#            print(f'max_word_len:{max_word_len}')
#            print(f'{target_padded_chars[1:].shape}')
            target_chars = target_padded_chars[1:].contiguous().view(-1, max_word_len) # 句子里面的所有字符
